...
namespace ITP_StateMachine.Device
{
    ...
    public class Hid : object
    {
        ...
        {
            //vID = 0x0ACD, pID = 0x0500 for ID TECH Duo 
            ...
            if (deviceOpened == false)
            {
                ...
                for (int i = 0; i < deviceList.Count; i++)
                {
                    IntPtr device = CreateFile(deviceList[i],
                                                ...
                                                FILE_ACCESS.FILE_SHARE_READ | FILE_ACCESS.FILE_SHARE_WRITE,
                                                0,
        ...
        private void ReadCompleted(IAsyncResult iResult)
        {
            ...
            catch
            {
                ...
                OnDeviceRemoved(ex);//
                CloseDevice();
                //CardDetails.CardNumber = "Device error.";
            }
        }

        private bool ParseData(byte[] ReceivedCardData)
        {
            bool bT2StartSentinelFound = false;

            byte[] track2 = new byte[21];
            int iFolioPointer = 0;

            const int MAXEXPECTEDDATALENGTH = 150;
            const int STARTTRACKDATA = 9;
            int iFolioStartPointer = 0;
            string rawdata = "";


            for (int i = STARTTRACKDATA; i < MAXEXPECTEDDATALENGTH; i++)
            {
                char sThisChar = Convert.ToChar(ReceivedCardData[i]);

                rawdata = rawdata + sThisChar;

                // Find the start of track 2 data (; = 59 or 3B5H)
                if (sThisChar.Equals(';'))
                {
                    bT2StartSentinelFound = true;
                    iFolioStartPointer = i + 1;         // Store pointer to first expected digit
                }

                if (bT2StartSentinelFound == true)
                {
                    if (i == iFolioStartPointer)
                    {
                        if (Char.IsNumber(sThisChar) == false)
                        {
                            bT2StartSentinelFound = false;  // Reset flag as this was not a valid start sentinel
                        }
                    }
                    // Extract digits
                    if (Char.IsNumber(sThisChar) == true || sThisChar.Equals('='))
                    {

                        track2[iFolioPointer] = ReceivedCardData[i];
                    ...
                    else
                    {
                        if (sThisChar.Equals('?'))
                        {
                            string value = System.Text.ASCIIEncoding.ASCII.GetString(track2);

                            //Globals.AddDebugLine("Card swiped, Track2: " + value);
                            //System.Diagnostics.Debug.WriteLine("Card swiped, Track2: " + value);

                            //string[] track2Values = value.Split('=');
                            //Globals.AddDebugLine("      Corp ID: " + track2Values[0]);
                            //System.Diagnostics.Debug.WriteLine("      Corp ID: " + track2Values[0]);

                            //Globals.AddDebugLine("      AccountNumber: " + track2Values[1]);
                            //System.Diagnostics.Debug.WriteLine("      AccountNumber: " + track2Values[1]);

                            OnDataReceived(rawdata + "|" + value);
        ...
        public delegate void DelegateDataReceived(object sender, string e);
