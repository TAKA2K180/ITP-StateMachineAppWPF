...
namespace Stateless.Tests
{
    public class StateMachineFixture
    {
        ...
        void RunSimpleTest<TState, TTransition>(IEnumerable<TState> states, IEnumerable<TTransition> transitions)
        {
            ...
            var sm = new Classes.StateMachine<TState, TTransition>(a);
        ...
        {
            var initial = State.B;
            var sm = new Classes.StateMachine<State, Trigger>(initial);
        ...
        {
            var state = State.B;
            var sm = new Classes.StateMachine<State, Trigger>(() => state, s => state = s);
        ...
        public void StateMutatorShouldBeCalledOnlyOnce()
        {
            var state = State.B;
            var count = 0;
            var sm = new Classes.StateMachine<State, Trigger>(() => state, (s) => { state = s; count++; });
            sm.Configure(State.B).Permit(Trigger.X, State.C);
            sm.Fire(Trigger.X);
        ...
        {
            var sm = new Classes.StateMachine<State, Trigger>(State.B);
        ...
        {
            var sm = new Classes.StateMachine<State, Trigger>(State.A);
        ...
        public void WhenATransitionOccurs_TheOnTransitionedEventFires()
        {
            ...
            Classes.StateMachine<State, Trigger>.Transition transition = null;
        ...
        {
            Classes.StateMachine<State, Trigger> sm = new Classes.StateMachine<State, Trigger>(State.B);
        ...
        public void PermittedTriggersIncludeAllInheritedTriggers()
        {
            ...
            Classes.StateMachine<State, Trigger> hsm(State initialState)
                => new Classes.StateMachine<State, Trigger>(initialState)
