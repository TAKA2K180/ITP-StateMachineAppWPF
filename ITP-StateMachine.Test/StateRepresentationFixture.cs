...
namespace Stateless.Tests
{
    ...
    public class StateRepresentationFixture
    {
        ...
        public void UponEntering_EnteringActionsExecuted()
        {
            ...
            Classes.StateMachine<State, Trigger>.Transition
                transition = new Classes.StateMachine<State, Trigger>.Transition(State.A, State.B, Trigger.X),
                actualTransition = null;
            stateRepresentation.AddEntryAction((t, a) => actualTransition = t, Classes.Reflection.InvocationInfo.Create(null, "entryActionDescription"));
        ...
        public void UponLeaving_LeavingActionsExecuted()
        {
            ...
            Classes.StateMachine<State, Trigger>.Transition
                transition = new Classes.StateMachine<State, Trigger>.Transition(State.A, State.B, Trigger.X),
                actualTransition = null;
            stateRepresentation.AddExitAction(t => actualTransition = t, Classes.Reflection.InvocationInfo.Create(null, "entryActionDescription"));
            stateRepresentation.Exit(transition);
            Assert.Equal(transition, actualTransition);
        }
        ...
        public void UponEntering_LeavingActionsNotExecuted()
        {
            ...
            Classes.StateMachine<State, Trigger>.Transition
                transition = new Classes.StateMachine<State, Trigger>.Transition(State.A, State.B, Trigger.X),
                actualTransition = null;
            stateRepresentation.AddExitAction(t => actualTransition = t, Classes.Reflection.InvocationInfo.Create(null, "exitActionDescription"));
            stateRepresentation.Enter(transition);
            Assert.Null(actualTransition);
        }
        ...
        {
            CreateSuperSubstatePair(out Classes.StateMachine<State, Trigger>.StateRepresentation super, out Classes.StateMachine<State, Trigger>.StateRepresentation sub);

            var executed = false;
            sub.AddEntryAction((t, a) => executed = true, Classes.Reflection.InvocationInfo.Create(null, "entryActionDescription"));
            var transition = new Classes.StateMachine<State, Trigger>.Transition(super.UnderlyingState, sub.UnderlyingState, Trigger.X);
        ...
        public void WhenTransitioningFromSubToSuperstate_SubstateExitActionsExecuted()
        {
            ...
            var executed = false;
            sub.AddExitAction(t => executed = true, Classes.Reflection.InvocationInfo.Create(null, "exitActionDescription"));
            var transition = new Classes.StateMachine<State, Trigger>.Transition(sub.UnderlyingState, super.UnderlyingState, Trigger.X);
            sub.Exit(transition);
            Assert.True(executed);
        }
        ...
        public void WhenTransitioningToSuperFromSubstate_SuperEntryActionsNotExecuted()
        {
            ...
            var executed = false;
            super.AddEntryAction((t, a) => executed = true, Classes.Reflection.InvocationInfo.Create(null, "entryActionDescription"));
            var transition = new Classes.StateMachine<State, Trigger>.Transition(super.UnderlyingState, sub.UnderlyingState, Trigger.X);
        ...
        public void WhenTransitioningFromSuperToSubstate_SuperExitActionsNotExecuted()
        {
            ...
            var executed = false;
            super.AddExitAction(t => executed = true, Classes.Reflection.InvocationInfo.Create(null, "exitActionDescription"));
            var transition = new Classes.StateMachine<State, Trigger>.Transition(super.UnderlyingState, sub.UnderlyingState, Trigger.X);
        ...
        public void WhenEnteringSubstate_SuperEntryActionsExecuted()
        {
            ...
            var executed = false;
            super.AddEntryAction((t, a) => executed = true, Classes.Reflection.InvocationInfo.Create(null, "entryActionDescription"));
            var transition = new Classes.StateMachine<State, Trigger>.Transition(State.C, sub.UnderlyingState, Trigger.X);
            sub.Enter(transition);
            Assert.True(executed);
        }
        ...
        public void WhenLeavingSubstate_SuperExitActionsExecuted()
        {
            ...
            var executed = false;
            super.AddExitAction(t => executed = true, Classes.Reflection.InvocationInfo.Create(null, "exitActionDescription"));
            var transition = new Classes.StateMachine<State, Trigger>.Transition(sub.UnderlyingState, State.C, Trigger.X);
            sub.Exit(transition);
            Assert.True(executed);
        }
        ...
        public void EntryActionsExecuteInOrder()
        {
            ...
            rep.AddEntryAction((t, a) => actual.Add(0), Classes.Reflection.InvocationInfo.Create(null, "entryActionDescription"));
            rep.AddEntryAction((t, a) => actual.Add(1), Classes.Reflection.InvocationInfo.Create(null, "entryActionDescription"));

            rep.Enter(new Classes.StateMachine<State, Trigger>.Transition(State.A, State.B, Trigger.X));
        ...
        public void ExitActionsExecuteInOrder()
        {
            ...
            var rep = CreateRepresentation(State.B);
            rep.AddExitAction(t => actual.Add(0), Classes.Reflection.InvocationInfo.Create(null, "entryActionDescription"));
            rep.AddExitAction(t => actual.Add(1), Classes.Reflection.InvocationInfo.Create(null, "entryActionDescription"));

            rep.Exit(new Classes.StateMachine<State, Trigger>.Transition(State.B, State.C, Trigger.X));
        ...
        {
            var rep = CreateRepresentation(State.B);
            rep.AddTriggerBehaviour(new Classes.StateMachine<State, Trigger>.IgnoredTriggerBehaviour(Trigger.X, null));
        ...
        public void WhenEnteringSubstate_SuperstateEntryActionsExecuteBeforeSubstate()
        {
            ...
            super.AddEntryAction((t, a) => superOrder = order++, Classes.Reflection.InvocationInfo.Create(null, "entryActionDescription"));
            sub.AddEntryAction((t, a) => subOrder = order++, Classes.Reflection.InvocationInfo.Create(null, "entryActionDescription"));
            var transition = new Classes.StateMachine<State, Trigger>.Transition(State.C, sub.UnderlyingState, Trigger.X);
            sub.Enter(transition);
        ...
        public void WhenExitingSubstate_SubstateEntryActionsExecuteBeforeSuperstate()
        {
            ...
            int order = 0, subOrder = 0, superOrder = 0;
            super.AddExitAction(t => superOrder = order++, Classes.Reflection.InvocationInfo.Create(null, "entryActionDescription"));
            sub.AddExitAction(t => subOrder = order++, Classes.Reflection.InvocationInfo.Create(null, "entryActionDescription"));
            var transition = new Classes.StateMachine<State, Trigger>.Transition(sub.UnderlyingState, State.C, Trigger.X);
            sub.Exit(transition);
        ...
        public void WhenTransitionUnmetGuardConditions_TriggerCannotBeFired()
        {
            ...
            var transitionGuard = new Classes.StateMachine<State, Trigger>.TransitionGuard(falseConditions);
            var transition = new Classes.StateMachine<State, Trigger>.TransitioningTriggerBehaviour(Trigger.X, State.C, transitionGuard);
        ...
        public void WhenTransitioGuardConditionsMet_TriggerCanBeFired()
        {
            ...
            var transitionGuard = new Classes.StateMachine<State, Trigger>.TransitionGuard(trueConditions);
            var transition = new Classes.StateMachine<State, Trigger>.TransitioningTriggerBehaviour(Trigger.X, State.C, transitionGuard);
            rep.AddTriggerBehaviour(transition);
        ...
        public void WhenTransitionExistAndSuperstateUnmetGuardConditions_FireNotPossible()
        {
            ...
            var reslt= sub.TryFindHandler(Trigger.X, new object[0], out Classes.StateMachine<State, Trigger>.TriggerBehaviourResult result);
        ...
        public void WhenTransitionExistSuperstateMetGuardConditions_CanBeFired()
        {
            ...
            super.AddTriggerBehaviour(transition);
            sub.TryFindHandler(Trigger.X, new object[0], out Classes.StateMachine<State, Trigger>.TriggerBehaviourResult result);
        ...
        void CreateSuperSubstatePair(out Classes.StateMachine<State, Trigger>.StateRepresentation super, out Classes.StateMachine<State, Trigger>.StateRepresentation sub)
        {
        ...
        Classes.StateMachine<State, Trigger>.StateRepresentation CreateRepresentation(State state)
        {
            return new Classes.StateMachine<State, Trigger>.StateRepresentation(state);
        }
        ...
        public void SetGuardDescriptionWhenSubstateGuardFails()
        {
            ...
            var fsm = new Classes.StateMachine<State, Trigger>(State.B);
        ...
        public void AddAllGuardDescriptionsWhenMultipleGuardsFailForSameTrigger()
        {
            ...
            var fsm = new Classes.StateMachine<State, Trigger>(State.A);
            fsm.OnUnhandledTrigger((state, trigger, descriptions) => guardDescriptions = descriptions);
